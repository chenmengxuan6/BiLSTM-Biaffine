    ...
    def get_labels(self, args):
        ...
        if os.path.exists(os.path.join(args.output_dir, "label_list.pkl")):
            ...
            if len(labels) > 0:
                ...
                labels = {'O', 'B-protein', 'B-cell_type', 'B-DNA', 'B-RNA', 'B-cell_line', 'I-protein', 'I-cell_type', 'I-DNA', 'I-RNA', 'I-cell_line','E-protein', 'E-cell_type', 'E-DNA', 'E-RNA', 'E-cell_line','S-protein', 'S-cell_type', 'S-DNA', 'S-RNA', 'S-cell_line'}
                # labels = {'O', 'B', 'I'}
        return labels 
...
def convert_examples_to_features(args, examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in tqdm(enumerate(examples), desc="convert examples"):
        ...
        for i, word in enumerate(textlist):
            ...
            for m in range(len(token)):
                ...
                # label_ids.append(label_map[label1])


            sentence_word.append(len(token))
        ...
        if len(tokens) >= max_seq_length - 1:
            ...
            # if sum(sentence_word) > 256:
            #     print('Error')
            #     for ss in range(0, len(sentence_word)):
            #         if sum(sentence_word[0:ss]) == 256:
            #             sentence_word = sentence_word[0:ss]
            #             break


        ori_tokens = ["[CLS]"] + ori_tokens + ["[SEP]"]
        sentence_word.append(1)
        sentence_word.insert(0,1)
        ...
        if len(input_ids) < max_seq_length:
            ...
            sentence_word.append(padding_length)
    ...
    return features,sentences_word
...
def get_Dataset(args, processor, tokenizer, mode="train"):
    ...
    features,sentences_word= convert_examples_to_features(
    ...
    return examples, features, data, sentences_word, len(data)
