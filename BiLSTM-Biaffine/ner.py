...
def evaluate(args, data, model, id2label, all_ori_tokens, eval_sentences_word, eval_len_data):
    model.eval()
    ...
    for b_i, (input_ids, input_mask, segment_ids, label_ids) in enumerate(tqdm(dataloader, desc="Evaluating")):
        ...
        with torch.no_grad():
            logits = model.predict(input_ids, b_i, eval_len_data, segment_ids, input_mask, eval_sentences_word)
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--train_file", default="dataset/BC5CDR-chem-IOB/train.tsv", type=str)
    parser.add_argument("--eval_file", default="dataset/BC5CDR-chem-IOB/devel.tsv", type=str)
    parser.add_argument("--test_file", default="dataset/BC5CDR-chem-IOB/test.tsv", type=str)
    parser.add_argument("--model_name_or_path", default="biobert_v1.1_pubmed", type=str)
    parser.add_argument("--output_dir", default="output", type=str)

    ## other parameters
    parser.add_argument("--config_name", default="biobert_v1.1_pubmed/config.json", type=str,
                        help="Pretrained config name or path if not the same as model_name")
    parser.add_argument("--tokenizer_name", default="", type=str,
                        help="Pretrained tokenizer name or path if not the same as model_name") ################
    parser.add_argument("--cache_dir", default="output/behave", type=str,
    ...
    if args.do_train:
        ...
        train_examples, train_features, train_data, train_sentences_word, train_len_data= get_Dataset(args, processor, tokenizer, mode="train")
        train_sampler = RandomSampler(train_data)
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size)

        if args.do_eval:
            eval_examples, eval_features, eval_data, eval_sentences_word, eval_len_data= get_Dataset(args, processor, tokenizer, mode="eval")
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                # input_ids, input_mask, segment_ids, label_ids, train_sentences_word, train_len_data = batch
                input_ids, input_mask, segment_ids, label_ids = batch
                outputs = model(input_ids, step, train_len_data, label_ids, segment_ids, input_mask,train_sentences_word)
                loss = outputs
            ...
            if args.do_eval:
                ...
                overall, by_type = evaluate(args, eval_data, model, id2label, all_ori_tokens_eval, eval_sentences_word, eval_len_data)
                ...
                with open(os.path.join(args.output_dir, "eval1"), "a") as f:
                    f.write(str(overall.fscore))
                f.close()
    ...
    if args.do_test:
        model = torch.load(os.path.join(args.output_dir, 'model.bin'))
        model.to(device)

        test_examples, test_features, test_data, test_sentences_word, test_len_data = get_Dataset(args, processor, tokenizer, mode="test")
        ...
        overall, by_type = evaluate(args, test_data, model, id2label, all_ori_tokens_test,test_sentences_word, test_len_data)
        f1_score = overall.fscore
        print(f1_score)
